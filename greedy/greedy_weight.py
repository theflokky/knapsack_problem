...
def insertion_sort_01(list, left, right):
    for i in range(left + 1, right + 1):
        j = i
        while j > left and list[j][1] < list[j - 1][1]:
...
def greedy_weight_01(lo, w):
    ...
    #Newest version using my own implementation of the TimSort algorithm, the best one
    timsort_01(lot)
...
# HERE LIES THE MULTIDIMENSIONAL VERSION
...
def insertion_sort_multi(list, left, right):
    for i in range(left + 1, right + 1):
        j = i
        while j > left and (sum(list[j][1])/len(list[j][1])) < (sum(list[j - 1][1])/len(list[j][1])):
            list[j], list[j-1] = list[j - 1], list[j]
            j -= 1
...
def merge_multi(list, l, m, r):
    ...
    # After comparing the list, we merge them
    while i < len1 and j < len2:
        if sum(left[i][1])/len(left[i][1]) > sum(right[j][1])/len(right[j][1]):
            list[k] = right[j]
            j+=1
        else:
            list[k] = left[i]
